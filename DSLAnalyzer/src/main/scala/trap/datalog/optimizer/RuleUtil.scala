...
object RuleUtil {
  ...
  def getRuleMap(ruleString: String) = {
    ...
    val rules = cleanedString.filterNot(_.startsWith("//")).foldLeft("")((x, y) => x + y).split('.').filterNot(_.isEmpty).map(_ + ".")
    // each element of rule is a string of type "foo(?x,?y):-foo(?y,?z),bar(?z,?x),?x='foo'."
    val headTailMap = rules.map(_.split(":-")).map(x => (getName(x(0)), x(1).split(',').filter(_.contains('(')).map(getName).toSet))
