...
object RuleUtil {
  val splChar = '$'
  def paraMatch(p:ParaDef, v:IRISType) = (p.paraType, v) match {
...
//    case (ParaStringType, IRISAny(_)) => true
  ...
  def typeMatch(left:IRISType, right:IRISType) = (left, right) match {
  ...
  def getRel(s:String) = {
    ...
    IRISRel(name, ar(1).split(',') map(_ match {
...
| 	{ternary} 	[first]: term (+,*,-./) [second]: term (=) [third]: term
  ...
  def getVars(s:Seq[IRISType]) = s collect {
